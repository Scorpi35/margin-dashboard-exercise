...
export const errorHandler: ErrorRequestHandler = (err, req, res, _next) => {
  ...
  const unavailable = !isHttpError && isDatabaseUnavailable(err);
  const databaseFailed = !isHttpError && !unavailable && isDatabaseError(err);
  ...
  const statusCode = isHttpError ? err.statusCode : unavailable || databaseFailed ? 503 : 500;
